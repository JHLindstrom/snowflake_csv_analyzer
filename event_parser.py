...
def calculate_funnel(file_path: str, steps: List[str], delimiter: str = "->") -> pd.DataFrame:
    """
    Calculates sequential funnel drop-off metrics for a given sequence of steps.
    """
    if not steps:
    ...
    # Build regex patterns for sequential matching
    funnel_data = []
    prev_count = None
    first_count = None
    ...
    for idx, step in enumerate(clean_steps):
        ...
        if idx == 0:
            ...
            conversion_pct = round((count / prev_count) * 100, 2) if prev_count > 0 else 0.0
            dropoff_pct = round(100.0 - conversion_pct, 2)
            prev_count = count

        overall_pct = round((count / first_count) * 100, 2) if first_count > 0 else 0.0
